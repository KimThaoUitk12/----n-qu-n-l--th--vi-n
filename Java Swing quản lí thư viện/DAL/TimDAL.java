...
public class TimDAL {
    ...
    public ArrayList<TimDTO> getTim(String tensach,String tentg,String theloai, String nxb,String trangthai){
        ArrayList<TimDTO> ar=new ArrayList<>();
        String sql="select * from sach join nhaxuatban on nhaxuatban.manxb=sach.manxb where TENSACH LIKE '%" + tensach + "%' AND TACGIA LIKE '%" + tentg + "%' AND TENNXB LIKE '%" + nxb + "%' AND THELOAI LIKE '%" + theloai + "%' AND TRANGTHAI LIKE '%"+trangthai+"%'";
        ResultSet rs=DAL.Database.getData(conectionJDBC(), sql);
        try {
            while(rs.next()){
                TimDTO tim=new TimDTO();
