...
public class TraDAL {
    ...
    public void TraSach(TraDTO tra,int index,String manv){
        if (index < 0) {
       ...
       // PreparedStatement st;
        ...
        try {
            String mapp=Database.TaoMa("phieuphat");
            CallableStatement st=conectionJDBC().prepareCall(sql);
            st.setString(1, tra.getMaSach());
            st.setString(2, tra.getMaDG());
            st.setString(3, mapp);
            st.setString(4, manv);
            st.setString(5, "");
            st.execute();
...
//            ex.printStackTrace();
//        }
        String sql="{call CHOMUON_SACH(?,?,?)}";
        try {
            CallableStatement st=conectionJDBC().prepareCall(sql);
            st.setString(1, mapm);
            st.setString(2, bt.getMaSach());
            st.setString(3, madg);
            st.execute();
        } catch (SQLException ex) {
            Logger.getLogger(TraDAL.class.getName()).log(Level.SEVERE, null, ex);
